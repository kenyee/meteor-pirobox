...
(function($) {
	...
	for (var i=0; i<gall_settings.length; i++) {
		...
		if(gall_settings[i] == '.pirobox' || gall_settings[i]  == 1){
			
			$(gall_settings[i]).addClass('single_fix');
			
		}else if (gall_settings.length > 1){
		...
		function open_all(){
				...
				wrapper.animate({
					...
					'left':  ((x/2)-((params[1])/2+opt.padding))
							...
							if($('.piro_caption p').height()> 28){
								$('.piro_caption p').css({'background':'url(/packages/pirobox/lib/css/style_8/caption_up_down.png) top right no-repeat','padding-right':'18px'});
			...
			function animate_image (){
				...
				img.onerror = function (){
					...
					img.src = "/packages/pirobox/lib/js/error.jpg";
					img.width = '368';
					img.height = '129';
				};
				img.onload = function() {
							...
							$(img).fadeIn(700,function(){
								...
								$('.piro_caption').show(0,function(){
										...
										if($('.piro_caption p').height() > 28 || $('.piro_caption p').width() > imgW-(43*piro_nav_length) ){
											$('.piro_caption p').css({
												'background':'url(/packages/pirobox/lib/css/style_8/caption_up_down.png) top right no-repeat',
												'padding-right':'18px'
											});
